...
def verify_featured_nodes(
    base_nodes: pd.DataFrame,
    featured_nodes: pd.DataFrame,
    node_schema: dict,
) -> None:
    """Check that feature attachment preserved node identity and numeric fields."""
    ...
    feature_cols = [col for col in featured_nodes.columns if col not in metadata_cols]

    assert feature_cols, "No feature columns found"
    assert featured_nodes[feature_cols].notna().all().all(), (
        "Missing numeric feature values in node table"
    )
    assert all(featured_nodes[col].dtype.kind in "iuf" for col in feature_cols), (
