...
def build_occupation_descriptor_features(
    edges: pd.DataFrame,
    descriptor_config: dict,
) -> pd.DataFrame:
    """Aggregate edge statistics into occupation-level features."""
    ...
    basic_occupation_descriptor_features = (
        edges.groupby("occupation_idx").agg(**agg_map).reset_index()
    )
...
def build_descriptor_features(
    ...
    """Aggregate edge statistics into descriptor-level features."""

    idx_col = descriptor_config["idx_col"]

    basic_descriptor_features = (
        edges.groupby(idx_col)
        .agg(
...
def attach_features_to_nodes(
    nodes: pd.DataFrame,
    features: pd.DataFrame,
    on_col: str,
) -> pd.DataFrame:
    """Attach feature rows to a node table."""
...
def fill_missing_feature_values(
    ...
    """Fill missing feature values with zeroes."""

    node_table = node_table.copy()
    feature_cols = [col for col in node_table.columns if col not in metadata_cols]
